...
|---|---|---|---|---|---|---|
| Wiki ページビュー | `src/plugins/wiki/View.vue` | `/wiki/pages/<slug>` 単独 / `/chat` の manageWiki tool result | `marked` + `v-html` | pre-parse rewriter(basePath = `data/wiki/pages/`) | `data/wiki/pages/<slug>.md` の file-change pub/sub を購読して自動 refresh | 閲覧・スクロール / タスクチェックボックス編集(自動保存)/ 印刷 / **PDF ダウンロード** / Wiki 内部リンクの内部 routing / frontmatter 表示 / Content↔History タブ |
| Wiki インデックス | 同上 | `/wiki`, `/wiki/index` | カードリスト(markdown 未 parse) | 不要(画像なし) | `data/wiki/index.md` の file-change pub/sub を購読して自動 refresh | 閲覧・スクロール / タグフィルタ(URL 未永続)/ lint chat ボタン |
| Wiki ログビュー | 同上 | `/wiki/log` | `marked` + `v-html` | pre-parse rewriter | `data/wiki/log.md` の file-change pub/sub を購読して自動 refresh | 閲覧のみ |
| Wiki Lint レポート | 同上 | `/wiki/lint-report` | `marked` + `v-html` | pre-parse rewriter | 現状は部分的。index 更新は拾うが、page 群全体の再計算を一括で拾う broad channel は未実装 | 閲覧のみ |
