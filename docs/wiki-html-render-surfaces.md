...
| 2. **SPA 側 rewriter**| markdown は `marked` の walker で(`rewriteMarkdownImageRefs`、basePath 付き)、HTML は post-LLM 正規表現で(`rewriteHtmlImageRefs`)、すべて `resolveImageSrc()` を通して URL を正規化 | `src/utils/image/{rewriteMarkdownImageRefs,rewriteHtmlImageRefs,resolve}.ts` |
...
| 4. **ブラウザ self-repair**| `<img>` が 404 を返したとき、`src` に `artifacts/images/<rest>` が含まれていれば、その手前を切り落として `/artifacts/images/<rest>` で再試行(1度だけ、`data-image-repair-tried` で再帰防止) | `useImageErrorRepair()` composable + presentHtml の iframe srcdoc に inline `<script>` 注入(PR #974) |
...
| presentHtml plugin(チャット) | `src/plugins/presentHtml/View.vue` | `/chat` の tool result | `<iframe :src>` で path-based mount(`/artifacts/html/`、Files HTML と共通ルート、PR #982 で `srcdoc → src` 移行済)+ sandbox は `allow-scripts` のみ | post-LLM regex rewriter + 404 修復 inline script(PR #974 と共通) | 閲覧 / **ソース切替** / **印刷 PDF** |
