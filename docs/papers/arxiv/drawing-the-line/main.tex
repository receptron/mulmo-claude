...
\begin{table}[tb]
\caption{Vocabulary added in August 2026, in order, and the app or template that could not be said without it. One row is not used by any of the twenty-six apps in Appendix~\ref{sec:appendix}: \texttt{assigneeField} was added for the platform's salon template. The last column says where each sentence is held: ``rules'' means a direct write to the database is refused; ``host'' means the host refuses the value before sending it, and a direct write is not; ``page query'' means the sentence narrows what a page is handed and grants no permission; \texttt{agents[]} is the one sentence held by nothing (\S\ref{sec:instance}).}
\label{tab:growth}
\small
\begin{tabular}{@{}lp{8.6cm}l@{}}
\toprule
sentence & what could not be said before & held by \\
\midrule
\texttt{assignee} + \texttt{assigneeField} & the stylist approves \emph{their own} bookings, not a colleague's & rules \\
\texttt{selfDelete} & a person withdraws and the place opens & rules \\
\texttt{mirror} / \texttt{mirrorOf} & a slot shows as taken without reading who took it & rules \\
\texttt{transitions} (draft/open/closed) & a live poll closes one question before opening the next & rules \\
\texttt{uidField} & a board names who holds a task without publishing their address & rules \\
\texttt{idIn} & a claim's identity is the task's, so a task is held once & rules \\
\texttt{agents[]}$^{*}$ & an AI sitting at the app is told its job by the app (prose) & nothing \\
\texttt{refIn} & no message into a topic that is not open & rules \\
\texttt{sealed} & a closed topic cannot be deleted, by anyone & rules \\
\texttt{selfUpdate} + \emph{frozen} fields & an article gets a typo fixed without becoming a second article & rules \\
\texttt{maxBytes} & a body has a size the page is told before sending & host \\
\texttt{ownRead} & a participant's page is handed only their own rows & page query \\
\bottomrule
\end{tabular}
\end{table}
