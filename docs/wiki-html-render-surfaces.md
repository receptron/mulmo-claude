...
> **Note (#989)**: `manageWiki` MCP ツールは廃止され、LLM が `data/wiki/pages/*.md` を直接 Write/Edit することで auto-render が走る形になりました。`src/plugins/wiki/index.ts` のプラグイン登録は historical 録画再生用に view-only で残存。新規セッションでは MCP からの呼び出し経路は使われません。
