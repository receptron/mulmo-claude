...
describe("toSchedulerResult", () => {
  ...
  it("returns null for unrelated path even if body is a valid item array", () => {
    ...
    assert.equal(toSchedulerResult("data/other/items.json", JSON.stringify(items)), null);
  });
  ...
  it("returns null when JSON is malformed", () => {
    assert.equal(toSchedulerResult(WORKSPACE_FILES.schedulerItems, "{not json"), null);
  });

  it("returns null when parsed value isn't an array", () => {
    assert.equal(toSchedulerResult(WORKSPACE_FILES.schedulerItems, '{"foo":1}'), null);
  });

  it("returns null when array contains an element missing required fields", () => {
    // Missing `title`
    const bad = [{ id: "a", createdAt: 1, props: {} }];
    assert.equal(toSchedulerResult(WORKSPACE_FILES.schedulerItems, JSON.stringify(bad)), null);
  });
