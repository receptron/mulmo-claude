...
\begin{itemize}
\item The line: risk-proportional freedom for one AI author, with the view and UI controller above it and the model and business logic below, so that the generated page is untrusted and the generated declaration is bounded and mechanically checkable (\S\ref{sec:line}).
...
\node[draw, dashed, black!60, inner xsep=26pt, inner ysep=8pt, fit=(above)(below), label={[lab, black!70]above:written by the AI}] {};
\node[draw, dashed, black!60, inner sep=8pt, fit=(enf), label={[lab, black!70, align=center]above:platform\\ compiler, ruleset, hosting}] {};
\end{tikzpicture}
...
\paragraph{The vocabulary is closed.} There is a finite list of things that can be said, and nothing else. There is no escape hatch, no general-purpose expression, no way to compute. The absence of a way out is the point: every program in the language is composed entirely of sentence forms whose semantics and permitted combinations are known to the checker, and there is no construct by which sentences compose into something the checker has not seen.
...
If the language below the line is closed, the code above it must be genuinely free, or the line has bought nothing. The freedom is real: the page is ordinary HTML, CSS and JavaScript, and the AI writes it as it would write any page. What prevents that freedom from extending to the data is the interface across the line.
...
Each verb is answered by a sentence of the declaration together with the asker's role in \texttt{members}: a stranger has only what \texttt{public.submit} grants, a member of the roster has what their role grants, and the sentences below say what each verb needs beyond that. \emph{Submit} is allowed by \texttt{public.submit}, which names the fields a submitter may set. \emph{Move} is allowed by \texttt{transitions}, or by \texttt{selfTransitions} when the mover is the record's own submitter. \emph{Hand} is allowed by \texttt{assigneeField}: it puts a named member of the roster into that field, and only a member of the roster may do it. \emph{Withdraw} is allowed by \texttt{selfDelete}, in the states it lists. \emph{Correct} is allowed by \texttt{selfUpdate}, which lists, per current status, the fields the record's own submitter may rewrite; the status and assignee fields are never among them, frozen fields are refused whatever the list says, and \texttt{maxBytes} applies to the values. A verb the declaration has no sentence for is refused, whichever page asked.
...
The page is also confined in the ordinary way---it runs in an isolated frame with no network of its own, no form submission or navigation to another origin, no pop-up windows, no way to reach the host it is embedded in except through the intent channel---but this confinement is secondary. Its purpose is to make the intent channel the \emph{only} channel. Once that is so, the question ``what can this page do to the data?'' has a short answer: it can ask for five things, and every one of them is decided elsewhere. Two consequences are worth stating for a reader with an adversarial page in mind. A page that wanted to take what it was handed elsewhere has no way out: it has no network of its own, and the only channel it has leads to the declaration, which will store a value in a field it permits and nowhere else. That is a real limit and not a seal: a field the page may write may have readers the page's viewer does not, and a page can copy what it was handed into such a field. What confinement blocks is egress off the platform and any write the declaration does not name. And an intent is a typed request with named arguments, not a query and not a script; text that arrives in a record, however it was composed, is stored as text, and there is no interpreter below the line for it to reach.
...
\begin{table}[tb]
\caption{Vocabulary added in August 2026, in order, with the commit that introduced each (in \texttt{@receptron/sharedapp}; $^{\dagger}$ in MulmoTerminal, before the compiler was extracted; \texttt{0.31.0} is a release) and the app or template that could not be said without it. Two rows do not appear in the sentences column of Appendix~\ref{sec:appendix}: \texttt{assigneeField} was added for the platform's salon template and no listed app uses it, and the \emph{correct} intent is a verb the page may use rather than a key in the declaration; the key it rests on, \texttt{selfUpdate}, was in the base vocabulary from the start, which is why apps built on 14 August already carry it. What 0.31.0 added was the page's way to use it, and the frozen fields that bound it. The last column says where each sentence is held: ``rules'' means a direct write to the database is refused; ``host'' means the host refuses the value before sending it, and a direct write is not; ``page query'' means the sentence narrows what a page is handed and grants no permission; \texttt{agents[]} is advisory metadata rather than normative vocabulary, and is held by nothing (\S\ref{sec:instance}).}
\label{tab:growth}
\footnotesize
...
08-26 \texttt{0.31.0} & \emph{correct} intent + \emph{frozen} fields & an article gets a typo fixed without becoming a second article & rules \\
...
Second, additions came in pairs of a sentence and a refusal. \texttt{refIn} arrived with the rule that refuses the write; \texttt{sealed} arrived with the rule that refuses the delete; the \emph{correct} intent arrived with the list of fields that are \emph{frozen} and the rule that refuses a write to them. A sentence that nothing could hold was not added, with the one exception already named; and where a sentence is held somewhere other than the rules, the place is written down, so that a reader of the declaration knows what a stranger with direct database access can and cannot do to it.
