...
| サーフェス | コード | ルート | 方式 | 画像パス処理 | 外部変更の反映 | 主な機能 |
|---|---|---|---|---|---|---|
| Wiki ページビュー(standalone) | `src/plugins/wiki/View.vue` → 共有 `WikiPageBody.vue`(`renderWikiPageHtml(body, baseDir)`) | `/wiki/pages/<slug>` | `marked` + `v-html` | pre-parse rewriter(basePath = `data/wiki/pages/`) | `data/wiki/pages/<slug>.md` の file-change pub/sub を購読して自動 refresh | 閲覧・スクロール / タスクチェックボックス編集(自動保存)/ 印刷 / **PDF ダウンロード** / Wiki 内部リンクの内部 routing / frontmatter 表示 / Content↔History タブ |
| Wiki ページ inline render(page-edit) | 同上 + `pageEditLoader.ts` | `/chat` の `page-edit` tool result(LLM が Claude Code 標準 Write/Edit で `data/wiki/pages/*.md` を編集すると hook 経由で auto fire、PR #955 / #963 / #989) | 同上(`WikiPageBody` を共有) | 同上 | tool result そのものが「編集発火 → snapshot push」イベント。canvas は snapshot を直接表示 | 閲覧・スクロール / Wiki 内部リンク。**snapshot → 404 → live page (+banner) → 404 → "deleted" placeholder** の fallback chain |
| Wiki インデックス | `src/plugins/wiki/View.vue` | `/wiki`, `/wiki/index` | カードリスト(markdown 未 parse) | 不要(画像なし) | `data/wiki/index.md` の file-change pub/sub を購読して自動 refresh | 閲覧・スクロール / タグフィルタ(URL 未永続)/ lint chat ボタン |
| Wiki ログビュー | 同上 | `/wiki/log` | `marked` + `v-html`(`renderWikiPageHtml`) | pre-parse rewriter | `data/wiki/log.md` の file-change pub/sub を購読して自動 refresh | 閲覧のみ |
| Wiki Lint レポート | 同上 | `/wiki/lint-report` | `marked` + `v-html`(`renderWikiPageHtml`) | pre-parse rewriter | 現状は部分的。index 更新は拾うが、page 群全体の再計算を一括で拾う broad channel は未実装 | 閲覧のみ |

> **Note (#989)**: `manageWiki` MCP ツールは廃止され、LLM が `data/wiki/pages/*.md` を直接 Write/Edit することで auto-render が走る形になっています。`src/plugins/wiki/index.ts` のプラグイン登録は historical 録画再生用に view-only で残存。新規セッションでは MCP からの呼び出し経路は使われません。
...
5. **Wiki Lint レポートはまだ完全 pub/sub 化されていない** — page / index / log は file-change pub/sub で自動 refresh するが、lint_report は単一ファイルに紐づかないため page 群全体の変化を一括で拾う広い通知面がない。必要なら wiki-wide の集約 channel を別途追加する。
