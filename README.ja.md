...
Claude Code との GUI チャット — さらに長期記憶、ビジュアルツール、メッセージングアプリ連携を搭載。

Claude Code とチャットすると、単なるテキストだけでなく **インタラクティブなビジュアル出力** が得られます: ドキュメント、スプレッドシート、マインドマップ、チャート、画像、フォーム、3D シーンなど。組み込みの個人 Wiki により、Claude には会話のたびに成長する **永続的な知識** が備わります。
...
1. **Role Manager** ロールに切り替える
2. Claude にカスタムロールの作成、または既存ロールの編集を頼む
3. プラグインチェックリストで `readXPost` および / または `searchX` を有効化
