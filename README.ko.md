...
Claude Code와의 GUI 채팅 — 장기 기억, 시각적 도구, 메시징 앱 접근 기능까지 포함.

Claude Code와 대화하면 단순한 텍스트가 아닌 **인터랙티브한 시각적 출력**을 받을 수 있습니다: 문서, 스프레드시트, 마인드맵, 차트, 이미지, 폼, 3D 장면 등. 내장된 개인 위키는 Claude에게 대화를 거듭할수록 성장하는 **지속적인 지식**을 제공합니다.
...
1. **Role Manager** 역할로 전환
2. Claude에게 커스텀 역할을 생성하거나 기존 역할을 편집하도록 요청
3. 플러그인 체크리스트에서 `readXPost` 및/또는 `searchX` 활성화
